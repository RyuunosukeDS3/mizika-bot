...
class Sonarr:
    ...
    def get_sonarr_series(self, torrent_id):
        page = 1
        query_string = "includeUnknownSeriesItems=false&includeSeries=false&includeEpisode=false"
        response = requests.get(
            f"{self.api_url}/queue?page={page}&pageSize=10&{query_string}", headers=self.headers)
    ...
    def refresh_monitored_downloads_command(self):
        response = requests.post(
            f"{self.api_url}/command", headers=self.headers, json={"name": "RefreshMonitoredDownloads"})

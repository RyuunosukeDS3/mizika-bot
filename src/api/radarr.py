...
class Radarr:
    ...
    def get_radarr_queue_by_torrent_id(self, torrent_id):
        ...
        response = requests.get(
            f"{self.api_url}/queue?page={page}&pageSize=10&includeMovie=true", headers=self.headers)
    ...
    def refresh_monitored_downloads_command(self):
        response = requests.post(f"{self.api_url}/command", headers=self.headers, json={

...
class DownloadSeries(commands.Cog):
    ...
    async def add_series_command(self, ctx, magnet_link=""):
        ...
        series_data = self.sonarr_api.get_sonarr_series(torrent_data.hash_string)

        if series_data:
            embed.title = f"Adicionado {series_data['title']}"
            embed.color = Color.green()
            embed.add_field(name="Status:", value="Monitorado", inline=False)
            ...
            embed.add_field(
                name="Motivo:",
                value="Série precisa primeiro ser solicitada via [Overseerr](https://overseerr.mizika.duckdns.org/)",
                inline=False,
            )

...
class DownloadMovie(commands.Cog):
    ...
    async def add_movie_command(self, ctx, magnet_link=""):
        ...
        if movie_data:
            ...
            embed.add_field(
                name="Qualidade:", value=movie_data["quality"]["quality"]["name"], inline=False)
            embed.set_thumbnail(
                url=movie_data['movie']['images'][0]['remoteUrl'])
            embed.set_footer(text=f"ID do torrent {torrent_data.hash_string}")
        else:
            embed.title = "Falha ao adicionar Torrent"
            embed.color = Color.red()
            embed.add_field(
                name="Motivo:", value="Filme precisa primeiro ser solicitado via [Overseerr](https://overseerr.mizika.duckdns.org/)", inline=False)
